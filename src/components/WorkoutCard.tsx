...
export function WorkoutCard({workout}:{workout:Workout}){
  const {t}=useI18n(); const theme=statusTheme[workout.status]??statusTheme.PLANNED;
  const ionIcon = iconFor(workout.type);
  return <Pressable style={[s.card,{backgroundColor:theme.bg,borderColor:theme.border}]} onPress={()=>router.push(`/workout/${workout.id}`)}>
    ...
    <View style={{flex:1}}><Text style={[s.type,{color:theme.text}]}>{labels[workout.type]}</Text><Text style={s.meta}>{workout.date} · {workout.distanceKm} km{workout.targetPaceMinSec!=null?` · ${pace(workout.targetPaceMinSec)}–${pace(workout.targetPaceMaxSec)} /km`:''}</Text></View>
