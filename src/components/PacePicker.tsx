...
export function PacePicker({value,onChange,minMinute=3,maxMinute=15,compact=false}:Props){
 const {t}=useI18n(); const {colors}=useTheme(); const [open,setOpen]=useState(false); const initial=Math.max(0,Math.floor(value||450)); const [m,setM]=useState(Math.floor(initial/60)); const [sec,setSec]=useState(initial%60);
 ...
 return <>
  <Pressable style={[s.field,{backgroundColor:colors.bgCard,borderColor:colors.bgCardBorder},compact&&s.fieldCompact]} onPress={openPicker}>
    <View style={s.left}>
