...
function base64ToBytes(value: string) {
  ...
  if (
    ...
    throw new BackupFileError(
      "INVALID_FILE",
      "The backup contains invalid encoded data.",
    );
  }
...
function backupFileName() {
  ...
  return `Runmio-backup-${stamp}.rrbackup`;
}
...
function assertEncryptedBackup(
  value: unknown,
...
export async function backupToICloudDrive(
  ...
  if (password) {
    ...
    contents = await encryptPayload(payload, password, salt, nonce);
  } else {
