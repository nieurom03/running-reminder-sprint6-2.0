...
function weeksBetween(start: Date, end: Date) {
  return Math.max(1, Math.ceil((end.getTime() - start.getTime()) / (7 * DAY_MS)));
}
...
export function generateTrainingPlan(input: PlanInput, startDate = new Date()): GeneratedWorkout[] {
  ...
  for (let week = 0; week < totalWeeks; week++) {
    ...
    let longDistance = startLong + (peakLong - startLong) * Math.min(1, progress * 1.2);
    ...
    scheduledDates.forEach((d, index) => {
      ...
      let distance = clamp(Math.round((input.raceDistanceKm * 0.28 + week * 0.15) * 2) / 2, 4, input.raceDistanceKm <= 10 ? 7 : 9);
      ...
      if (isLong) {
        ...
        description = 'Long run: ưu tiên hoàn thành cự ly, kiểm soát nhịp tim và tiếp nước.';
      } else if (isQuality) {
        ...
        if (type === 'TEMPO') {
          ...
          description = 'Tempo: 1–2 km khởi động, phần giữa ở pace kiểm soát, sau đó thả lỏng.';
        } else {
          minPace = Math.max(220, input.currentPaceSec - 35);
          maxPace = input.currentPaceSec - 10;
          description = 'Interval: khởi động kỹ, chạy các đoạn nhanh ngắn xen kẽ hồi phục.';
        }
      } else if (index === scheduledDates.length - 1 && !isLong && selected.length >= 4) {
        type = 'RECOVERY';
      ...
      workouts.push({ date: iso(d), type, distanceKm: distance, targetPaceMinSec: minPace, targetPaceMaxSec: maxPace, description });
    });
  }
  ...
  workouts.push({
    ...
    targetPaceMinSec: Math.round((input.goalTimeMinutes * 60) / input.raceDistanceKm),
    targetPaceMaxSec: Math.round((input.goalTimeMinutes * 60) / input.raceDistanceKm) + 10,
    description: `RACE DAY · mục tiêu ${(()=>{const sec=Math.round(input.goalTimeMinutes*60);return `${Math.floor(sec/3600)}:${String(Math.floor((sec%3600)/60)).padStart(2,'0')}:${String(sec%60).padStart(2,'0')}`})()}`
  });
